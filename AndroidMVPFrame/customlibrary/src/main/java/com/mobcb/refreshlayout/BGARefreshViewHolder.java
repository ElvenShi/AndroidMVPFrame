 ...
 * @version [V1.0.0]
 * @date 2017/6/1 0001
 * @desc [继承该抽象类实现响应的抽象方法，做出各种下拉刷新效果。
...
public abstract class BGARefreshViewHolder {
     ...
     * 没有更多数据时显示在底部的
     */
    private View mLoadMoreFooterNoDataView;
    /**
     * 没有更多数据的TextView
     */
    private TextView mLoadMoreFooterNoDataTextView;
    /**
     * 没有更多数据的文字提醒
     */
    private String mNoMoreData = "没有更多了哟~~~";
    ...
    public boolean ismIsLoadingMoreEnabled() {
        return mIsLoadingMoreEnabled;
    }
    ...
    public View getNoMoreDataView() {
        if (mLoadMoreFooterNoDataView == null) {
            mLoadMoreFooterNoDataView = View.inflate(mContext, R.layout.view_normal_refresh_footer_no_data, null);
            mLoadMoreFooterNoDataView.setBackgroundColor(Color.TRANSPARENT);
            if (mLoadMoreBackgroundColorRes != -1) {
                mLoadMoreFooterNoDataView.setBackgroundResource(mLoadMoreBackgroundColorRes);
            }
            if (mLoadMoreBackgroundDrawableRes != -1) {
                mLoadMoreFooterNoDataView.setBackgroundResource(mLoadMoreBackgroundDrawableRes);
            }
            mLoadMoreFooterNoDataTextView = (TextView) mLoadMoreFooterNoDataView.findViewById(R.id.tv_normal_refresh_footer_no_data);
            mLoadMoreFooterNoDataTextView.setText(mNoMoreData);
        }
        return mLoadMoreFooterNoDataView;
    }
    ...
    public String getmNoMoreData() {
        return mNoMoreData;
    }

    public void setmNoMoreData(String mNoMoreData) {
