...
import com.mobcb.refreshlayout.util.BGARefreshScrollingUtil;
 ...
 * @version [V1.0.0]
 * @date 2017/6/1 0001
 * @desc [下拉刷新、上拉加载更多、可添加自定义（固定、可滑动）头部控件（例如慕课网app顶部的广告位）]
 */
public class BGARefreshLayout extends LinearLayout {
     ...
     * 上拉加载更多控件的高度
     */
    private int mNoMoreDataViewHeight;
    /**
     * 上拉加载更多控件的高度
     */
    ...
    private Runnable mDelayHiddenLoadingMoreViewTask = new Runnable() {
        ...
        public void run() {
            ...
            if (!hasMore) {
                mNoMoreDataView.setVisibility(VISIBLE);
            }
        }
    };
    ...
    public void setHasMore(boolean hasMore) {
        ...
        if (hasMore) {
            if (mNoMoreDataView != null) {
                mNoMoreDataView.setVisibility(GONE);
            }
        }
    }
    ...
    private void initNoMoreDataView() {
        mNoMoreDataView = mRefreshViewHolder.getNoMoreDataView();
        if (mNoMoreDataView != null) {
            mNoMoreDataView.measure(0, 0);
            mNoMoreDataViewHeight = mNoMoreDataView.getMeasuredHeight();
            mNoMoreDataView.setVisibility(GONE);
        }
    ...
    protected void onAttachedToWindow() {
        ...
        if (!mIsInitedContentViewScrollListener && mLoadMoreFooterView != null) {
            ...
            addView(mNoMoreDataView, getChildCount());
        ...
        if (mRecyclerView != null) {
            mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
                ...
                public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                    ...
                    Log.d("dx+dy", "dx=" + dx + ",dy=" + dy + ",position=" + positon);
                    if (mNoMoreDataView != null && !hasMore) {
                        positon += dy;
                        PropertyValuesHolder pvhY1_2 = PropertyValuesHolder.ofFloat("translationY", -positon);
                        ObjectAnimator.ofPropertyValuesHolder(mNoMoreDataView, pvhY1_2).setDuration(0).start();
